<template>
    <div class="profile-relations">
        ...
        <div class="profile-relations-users">
          ...
          <vue-horizontal-list v-else :items="allRelationsBadges(recommendations['all'])" :options="horizontalListOptions">
            <template v-slot:default="{item}">
              <a
                  ...
                  <div class="profile-relations__badged-picture">
                    <div v-if="item.explainer" v-tooltip="hoverExplainers[userId2HoverIndex(item.id)]" :class="badgeType(item)"></div>
                    <div v-else :class="badgeType(item)"></div>
                    <img v-if="!item.explainer" class="profile-relations__picture" :src="item.avatar_url" />
                  </div>
              </a>
            </template>
          </vue-horizontal-list>
...
export default {
  ...
  methods: {
    ...
    allRelationsBadges: function(recommendations) {
      let result = [];
      let explainers = [
      ...
      recommendations.forEach(function(item, _) {
        let user = item[1];
    ...
    badgeType: function(user) {
      if (user.explainer) return `profile-relations__badge-explainer profile-relations__badge-explainer--${this.colorFromScore(user.score)}`;

      return `profile-relations__color-badge profile-relations__color-badge--${this.colorFromScore(user.score)}`;
    },
    userId2HoverIndex: function(userId) {
